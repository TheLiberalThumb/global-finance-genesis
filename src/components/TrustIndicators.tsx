  ...
  return (
    <section className="section-padding bg-background relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {partnerships.map((partnership, index) => (
              <div
                ...
                className={`interactive-card p-8 rounded-2xl bg-card border border-rich-gold/20 
        ...
        <div className="mb-16 scroll-fade-up scroll-stagger-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {projects.map((project, index) => (
              <div
                key={project.title}
                className={`p-6 rounded-xl bg-card/50 border border-rich-gold/10 
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className={`text-xs px-2 py-1 rounded ${
                    ...
                    project.status === 'Active' ? 'bg-rich-gold/20 text-rich-gold' :
        ...
        {/* Geographic Presence */}
        <div className="scroll-fade-up scroll-stagger-3">
          <h3 className="text-center heading-sm text-rich-gold mb-12">
            Global Presence
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {locations.map((location, index) => (
              <div
                ...
                className={`text-center p-6 rounded-xl bg-dark-navy/40 
                  border border-rich-gold/20 hover:border-rich-gold/60 
                  hover:bg-dark-navy/60 transition-all duration-300 
                  scroll-fade-up scroll-stagger-${index + 4}`}
              >
                <div className="w-12 h-12 mx-auto mb-4 text-rich-gold flex items-center justify-center 
                  bg-rich-gold/10 rounded-full hover:bg-rich-gold/20 hover:scale-110 transition-all duration-300">
                  {location.icon}
                </div>
                <h4 className="font-inter font-semibold text-pure-white mb-1">
                  {location.city}
                </h4>
                <p className="text-sm text-light-gray mb-2">{location.country}</p>
                <p className="text-xs text-rich-gold font-medium">{location.role}</p>
              </div>
            ))}
          </div>
        </div>
